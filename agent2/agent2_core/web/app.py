...
app = FastAPI(title="Agent2 - Fan RPM & Active Cooling AI Agent")

agent_instance: FanCoolingAgent = None
...
def init_web_agent(agent: FanCoolingAgent):
...
@app.post("/api/thermal_spike")
async def inject_thermal_spike():
    if agent_instance:
        agent_instance.cooling_model.simulated_temp += 20.0
    return {"status": "Thermal spike injected (+20°C)"}
